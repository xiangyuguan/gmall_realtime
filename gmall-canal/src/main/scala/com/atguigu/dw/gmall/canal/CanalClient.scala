...
import com.atguigu.dw.gmall.canal.util.CanalHandler
...
object CanalClient {
  def main(args: Array[String]): Unit = {
    ...
    val connector: CanalConnector = CanalConnectors.newSingleConnector(new InetSocketAddress("node105", 11111), "example", "", "")
    // 2. 连接到 Canal
    connector.connect()
    // 3. 监控指定的表的数据的变化
    connector.subscribe("gmall-realtime.order_info")
    while (true) {
